...
def run():
   ...
   #     print("There's no .netrc file or the The endpoint isn't in the netrc file")  # noqa E501
    ...
    if (download_mode=='s3'):
        ...
        downloads_data = [[u['URL'] for u in r['umm']['RelatedUrls'] if u['Type'] == "GET DATA VIA DIRECT ACCESS"] for r in results['items']]
    else:
...
#                    print("Downloading: " + f)
                    ...
                    #upload(dataset, url, session, token, bucket_name, staging_area=None, chunk_size=25600, force_upload=False,retry=True):
#                    upload(f, session, token, "opera-dev-isl-fwd-hyunlee")
...
def upload(file_url, session, token, bucket_name, staging_area=None, chunk_size=25600, force_upload=False,
    ...
    try:
        try:
            ...
            # this is working..however, it won't work with large file
            c = boto.connect_s3()
        ...
        logger.error("Error while uploading {}: {}".format(url, str(e)))
        #print("Error while upload " + str(url))
        if retry is True:
            #logger.info("Retrying download of {}".format(url))
